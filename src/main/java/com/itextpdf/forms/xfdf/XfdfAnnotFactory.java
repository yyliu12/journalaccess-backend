...
public class XfdfAnnotFactory {
    ...
    private void addAnnotObjectAttribute(AnnotObject annotObject, Node attributeNode) {
        if (attributeNode != null) {
            ...
            if (attributeName == XfdfConstants.RECT) {
                System.out.println(attributeNode.getNodeValue());
                String[] rectValues = attributeNode.getNodeValue().split(",");
                float newx1 = Float.parseFloat(rectValues[0]) + 5;
                float newy1 = Float.parseFloat(rectValues[1]) + 5;

                attributeNode.setNodeValue(rectValues[0] + "," + rectValues[1] + "," +
                        newx1 + "," + newy1);
            }
